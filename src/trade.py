...
class Trade:
    ...
    def fn_lot_size(self,
                    risk,
                    llm_response):

        headers = {'Authorization': 'Bearer ' + self.oanda_token}

        acct_path = '/v3/accounts/' + self.oanda_acct
        response = requests.get('https://' + self.oanda_url + acct_path, headers=headers)
    ...
    def fn_place_trade(self,
        ...
        if take_profit > entry:
            ...
            print('ERROR with Trading Array')
            print(trading_array)
            df = pd.DataFrame([{
                'currency': currency,
                'buy_sell': None,
        ...
        if trade_type == 'MARKET_IF_TOUCHED':
            order = {'order': {
                'units': f'{units}',
                ...
                'type': trade_type,       # MARKET   LIMIT MARKET_IF_TOUCHED
        ...
        headers = {'Authorization': 'Bearer ' + self.oanda_token}
        orders_path = f'/v3/accounts/{self.oanda_acct}/orders'

        response_order = requests.post('https://' + self.oanda_url + orders_path, headers=headers, json=order)
    ...
    def fn_modify_trade(self,
                        currency,
                        trade_id,
                        response_llm
                        ):

        stay_exit = fn_extract_stay_exit(response_llm)
        ...
        if stay_exit == ['stay']:
            ...
            headers = {'Authorization': 'Bearer ' + self.oanda_token}
            orders_path = f'/v3/accounts/{self.oanda_acct}/trades/{trade_id}/close'
            response_order = requests.put('https://' + self.oanda_url + orders_path, headers=headers)
            print(f'Order Response: {response_order.status_code}')
    ...
    def fn_open_orders(self):
        # Orders
        headers = {'Authorization': 'Bearer ' + self.oanda_token}
        pricing_path = f'/v3/accounts/{self.oanda_acct}/orders'
        ...
        if response.json()['orders'] == []:
            ...
            if not df_orders.empty:
                ...
                try:
                    df_orders['trailingStopLoss'] = round(abs(df_orders['price'] - df_orders['trailingStopLossOnFill']['distance']), 5)
                except KeyError:
        ...
        # Trades
        headers = {'Authorization': 'Bearer ' + self.oanda_token}
        pricing_path = f'/v3/accounts/{self.oanda_acct}/trades'
        response = requests.get('https://' + self.oanda_url + pricing_path,
                                headers=headers
                                )
